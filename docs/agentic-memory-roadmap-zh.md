...
- `v3` layered composer 通过 `MEMORYOS_MEMORY_ARCH=v3` 显式启用，当前是 bench-candidate，不是默认路径。
...
- Core / Archival / Context Composer 已有 v3 契约和最小实现，但默认切换暂缓。
...
结论：保持 `v1` 默认，保留 v2/v3 opt-in。

原因：

- hard eval 仍然稳定，但 public v3 smoke 只证明诊断字段可运行。
- v3 还没有足够证据证明可以替代默认 `ContextBuilder` 路径。
- 旧 Page / Item / ContextBuilder 仍是当前 API/eval 兼容面。

完成标准：

- README / source / store / benchmark 文档反映 v1 默认、v2 opt-in、v3 bench-candidate。
