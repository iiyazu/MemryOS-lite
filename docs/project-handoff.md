...
3. **Qdrant+fastembed eval 卡死** — 启用 fastembed embedding 后，eval 反复卡在
   第一个 case（550 条消息 × ~50 pages）无法完成，原因未完全定位（可能是
   fastembed ONNX 推理与大量 page 循环的交互问题）。之前 80% 结果实际是
   embedding 维度不匹配导致 embedding 步骤被跳过，只用 BM25 检索得到的
