...
def test_v3_composer_scope_excluded_accounting_keeps_source_refs(tmp_path):
    ...
    excluded = next(
        ...
        if row["event_type"] == "archival_scope_excluded" and row["item_id"] == "apsg_excluded"
    )
...
def test_v3_composer_records_archival_vector_fallback_diagnostics(tmp_path):
    ...
    event_types = {row["event_type"] for row in package.metadata["component_accounting"]}
...
def test_v3_composer_filters_unattached_passage_before_archival_vector_search(tmp_path):
    ...
    assert package.metadata["archival_eligibility"]["selected_passage_ids"] == ["apsg_attached"]
    assert (
        "apsg_unattached" in package.metadata["archival_eligibility"]["scope_excluded_passage_ids"]
    )
...
def test_v3_composer_records_component_accounting_for_included_and_budget_dropped_items(
    ...
    dropped = next(row for row in accounting if row["item_id"] == "apsg_budget_dropped")
...
def test_v3_composer_final_trace_preserves_signed_packet_member_offsets(tmp_path):
    ...
    trace = package.metadata["final_context_trace"]
    anchor_row = next(
        row for row in trace if row["component"] == "recall" and row["item_id"] == "d1_anchor"
    )
...
def test_v3_composer_keeps_locomo_neighbor_in_same_benchmark_session(tmp_path):
    ...
    neighbor_rows = [
        row
        for row in package.metadata["final_context_trace"]
