...
def test_recall_pipeline_emits_unified_cache_metadata_on_hit(tmp_path) -> None:
    ...
    assert (
        second.metadata["query_analysis_cache"]["fallback_reason"] == "recall_context_package_hit"
    )
    assert second.metadata["recall_candidate_cache"]["status"] != "hit"
    assert (
        second.metadata["recall_candidate_cache"]["fallback_reason"] == "recall_context_package_hit"
    )
