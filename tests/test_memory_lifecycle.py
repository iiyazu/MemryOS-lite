...
def test_source_scoped_archival_memory_is_retrievable_by_source_scope(tmp_path):
    ...
    archival_items = [item for item in package.items if item.layer == "archival"]
    assert [item.text for item in archival_items] == ["Alice keeps a source-scoped rail note."]
    assert package.metadata["archival_eligibility"]["selected_passage_ids"] == ["apsg_amem_source"]
...
def test_archival_to_core_candidate_apply_persists_status_and_is_idempotent(tmp_path):
    store = _store(tmp_path)
    core = CoreMemoryService(store, TokenEstimator())
    service = MemoryLifecycleService(store, core)
    candidate = service.create_candidate(
        ...
        confidence=0.95,
        write_source="explicit_instruction",
        metadata={"label": "human", "limit_tokens": 100},
    )
    approved = ApprovalState(
        id="appr_1",
        session_id="ses_1",
        tool_name="core_candidate_apply",
        requested_action={"candidate_id": candidate.id},
        status="approved",
        requested_by="agent",
        approved_by="user",
        resolved_at=candidate.created_at,
    ...
    persisted = store.get_promotion_candidate(candidate.id)
    assert persisted is not None
    replayed = service.apply_candidate(persisted, actor="agent", approval_state=approved)

    assert applied.status == "applied"
    assert replayed.status == "applied"
    assert store.get_promotion_candidate(candidate.id).status == "applied"
