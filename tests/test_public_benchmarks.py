...
def test_public_benchmark_respects_explicit_embedding_provider_none(tmp_path, monkeypatch):
...
def test_cli_public_helpers_import_without_agent_answer_eval(monkeypatch):
    ...
    monkeypatch.delitem(importlib.sys.modules, "memoryos_lite.agent_answer_eval", raising=False)
...
def test_public_answerer_renders_structured_evidence_with_citation_contract(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_normalized_locomo_citations_reach_result_diagnostics(
    ...
    class FakeChatOpenAI:
        ...
        def invoke(self, messages):
            return type(
                "FakeResponse",
                (),
                {"content": ("Melanie planned to go camping around June 2023 [D2:7].")},
            )()
    ...
    evidence = [
        public_benchmarks.AnswerEvidence(
            evidence_id=full_id,
            text=(
                "[1:14 pm on 25 May, 2023] Melanie: I am thinking about going camping next month."
            ),
            component="recall",
            source_ids=[full_id],
            session_id="D2",
...
def test_public_answerer_brackets_parenthetical_allowed_citation_ids(tmp_path, monkeypatch):
...
def test_public_answerer_guides_relative_temporal_evidence_before_refusal(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_guides_supported_partial_evidence_before_refusal(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_guides_yes_no_inference_before_refusal(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_guides_complete_qualified_fact_answers(tmp_path, monkeypatch):
    captured_messages = []
        ...
        def invoke(self, messages):
            ...
            if "preserve explicit qualifiers" not in prompt_text:
                return type(
                    ...
                    {
                        "content": (
                            "I attended the University of Melbourne [3b6f954b:answer_94030872:009]."
                        )
                    },
                )()
            return type(
...
def test_public_answerer_omits_internal_diagnostics_metadata_from_prompt(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_repairs_missing_location_qualifier_after_retry(tmp_path, monkeypatch):
    class FakeChatOpenAI:
        def __init__(self, **kwargs):
            self.kwargs = kwargs
...
def test_public_answerer_retries_missing_qualifiers_after_refusal(tmp_path, monkeypatch):
    captured_prompts = []
...
def test_public_answerer_guides_relevant_evidence_over_refusal(tmp_path, monkeypatch):
    captured_messages = []
...
def test_public_answerer_retries_exact_refusal_when_evidence_exists(tmp_path, monkeypatch):
    captured_prompts = []
...
def test_public_answerer_retry_prompt_drops_exact_refusal_clause(tmp_path, monkeypatch):
    captured_prompts = []
    ...
    class FakeChatOpenAI:
        ...
        def invoke(self, messages):
            ...
            return type(
                "FakeResponse",
                (),
                {"content": ("Insufficient retrieved evidence to answer with source citations.")},
            )()
...
def test_public_answerer_recovers_temporal_holiday_answer_after_retry(tmp_path, monkeypatch):
    captured_prompts = []
        ...
        def invoke(self, messages):
            ...
            if "holiday names" in prompt_text and "month names" in prompt_text:
                return type(
                    ...
                    {
                        "content": (
                            "You volunteered on February 14th [58ef2f1c:answer_59547700:009]."
                        )
                    },
                )()
            return type(
...
def test_public_answerer_does_not_append_location_qualifiers_to_temporal_answer(
    ...
    class FakeChatOpenAI:
        ...
        def invoke(self, messages):
            return type(
                "FakeResponse",
                (),
                {"content": ("You volunteered on Valentine's Day [58ef2f1c:answer_59547700:009].")},
            )()
    ...
    assert answer == ("You volunteered on Valentine's Day [58ef2f1c:answer_59547700:009].")
                ...
                "final_context_trace": [
                    {
                        "component": "recall",
                        "item_id": "recall_item",
                        "source_refs": [{"source_type": "message", "source_id": "msg_expected"}],
                        "included": True,
                        "dropped": False,
                    }
                ],
                ...
                "final_context_trace": [
                    {
                        ...
                        "source_refs": [{"source_type": "message", "source_id": "msg_dropped"}],
                        "included": False,
                        "dropped": True,
                    }
                ],
...
def test_public_benchmark_v3_preserves_locomo_neighbor_sources_for_answer_evidence(
    ...
    neighbor_rows = [
        row for row in report["locomo_neighbor_diagnostics"] if row["metadata"].get("neighbor_of")
    ]
    assert expected_sources <= {row["item_id"] for row in neighbor_rows}
    assert {row["metadata"].get("benchmark_session_id") for row in neighbor_rows} == {"D7"}
...
def test_public_case_movement_from_comparison_report_pairs(tmp_path):
    ...
    assert summary["failure_classes"]["answer_not_supported_by_judge"] == ["case-new-failure-class"]
    assert summary["failure_boundaries"]["custom_boundary"] == ["case-new-failure-class"]
...
def test_public_case_movement_summary_reports_source_metric_movement_and_omits_missing_values():
        ...
        [
            {
                ...
                "source_hit": True,
                "case_diagnostics": {"baseline_source_metrics": {"source_hit": False}},
            },
            {
                "case_id": "metric-regressed",
                "source_hit": False,
                "case_diagnostics": {"baseline_source_metrics": {"source_hit": True}},
            },
            {
    ...
    assert "missing baseline or current metric values are omitted" in summary["diagnostic_note"]
...
def test_public_benchmark_writes_case_movement_summary_for_comparison_report(tmp_path):
    ...
    for metric in (
        ...
        assert summary["source_metric_movement"][metric]["improved"] == ["move_fail_to_pass_qa_001"]
...
def test_public_benchmark_resumes_from_partial_report(tmp_path, monkeypatch):
    data_path = tmp_path / "locomo_resume_two_cases.json"
    data_path.write_text(
        json.dumps(
            [
                {
                    "sample_id": "resume_case_one",
                    "conversation": {
                        "session_1": [
                            {
                                "speaker": "Alice",
                                "dia_id": "D1:1",
                                "text": "The first marker is amber.",
                            }
                        ],
                    },
                    "qa": [
                        {
                            "question": "What is the first marker?",
                            "answer": "amber",
                            "evidence": ["D1:1"],
                        }
                    ],
                ...
                {
                    "sample_id": "resume_case_two",
                    "conversation": {
                        "session_1": [
                            {
                                "speaker": "Bob",
                                "dia_id": "D1:1",
                                "text": "The second marker is teal.",
                            }
                        ],
                    },
                    "qa": [
                        {
                            "question": "What is the second marker?",
                            "answer": "teal",
                            "evidence": ["D1:1"],
                        }
                    ],
    ...
    def interrupted_run_baseline(
        ...
        **kwargs,
        ...
        call_case_ids.append(case.case_id)
        if case.case_id == "resume_case_two_qa_001" and not interrupted_once["value"]:
    ...
    monkeypatch.setattr(public_benchmarks, "_run_baseline", interrupted_run_baseline)
    with pytest.raises(RuntimeError, match="simulated interruption"):
        run_public_benchmark(
            settings,
            benchmark="locomo",
            data_path=data_path,
            run_id="resume-public-run",
            baselines=["memoryos_lite"],
            llm_answer=False,
            llm_judge=False,
        )

    partial_path = settings.data_dir / "evals" / "resume-public-run_locomo.partial.json"
    partial_rows = json.loads(partial_path.read_text(encoding="utf-8"))
    assert [row["case_id"] for row in partial_rows] == ["resume_case_one_qa_001"]
    ...
    assert call_case_ids == ["resume_case_two_qa_001"]
    assert [result.case_id for result in results] == [
        "resume_case_one_qa_001",
        "resume_case_two_qa_001",
    ]
...
def test_public_benchmark_respects_configured_rot_safe_budget(tmp_path, monkeypatch):
    data_path = _write_single_locomo_case(tmp_path, sample_id="sample_budget")
    ...
    def capture_budget(
        ...
        captured_budgets.append(budget_override)
        return original_run_baseline(
            baseline,
            case,
            messages,
    ...
    monkeypatch.setattr(public_benchmarks, "_run_baseline", capture_budget)
    settings = Settings(
        data_dir=tmp_path / ".memoryos",
        memoryos_memory_arch="v3",
        rot_safe_budget=7_000,
    )
    ...
    run_public_benchmark(
        ...
        benchmark="locomo",
        data_path=data_path,
        run_id="public-configured-budget-test",
        baselines=["memoryos_lite"],
        llm_answer=False,
        llm_judge=False,
    )

    assert captured_budgets == [7_000]
            ...
            if service.store.get_core_memory_block("core_public_profile") is None:
                service.store.create_core_memory_block(
                    CoreMemoryBlock(
                        ...
                        limit_tokens=100,
                        source_refs=[SourceRef(source_type="message", source_id=messages[0].id)],
                        tags=["profile"],
...
def test_public_benchmark_runs_kernel_step_when_v3_kernel_enabled(tmp_path):
    ...
    pending_event = next(
        event for event in kernel_trace_events if event["event_type"] == "approval_pending"
    )
    selected_events = [
        event for event in kernel_trace_events if event["event_type"] == "tool_selected"
    ]
    assert len(selected_events) == 2
    assert (
        pending_event["payload"]["metadata"]["tool_call_id"]
        == (selected_events[0]["payload"]["tool_call_id"])
    )
    assert (
        selected_events[1]["payload"]["tool_call_id"]
        == (selected_events[0]["payload"]["tool_call_id"])
    )
    ...
    granted_event = next(
        event for event in kernel_trace_events if event["event_type"] == "approval_granted"
    )
...
def test_planner_eval_sidecar_does_not_change_proposal_shape():
    model_visible = _model_visible_planner_input(citation_contract_status="supported_cited_answer")
            ...
            [
                {
                    ...
                    "maintenance_proposal": {
                        ...
                        "execution_mode": "proposal_only",
                        "tool_name": "archive_write",
                        "arguments": {"content": "Alice records a model-visible opt-in note."},
                        "source_refs": [{"source_type": "message", "source_id": source_id}],
                        "gold_fields_used": False,
                    },
                }
            ]
...
def test_public_repair_smoke_isolated_store_does_not_mutate_default_public_run(tmp_path):
    ...
    assert "tool_executed" in [event["event_type"] for event in repair_smoke["kernel_trace_events"]]

    assert data_path.read_text(encoding="utf-8") == original_data
    assert repair_artifact_text in (
        settings.data_dir / "eval_runs" / "repair-isolation-explicit" / "memoryos.db"
    ).read_text(encoding="utf-8", errors="ignore")
    assert repair_artifact_text not in json.dumps(default_after, sort_keys=True)
    assert repair_artifact_text not in (
        settings.data_dir / "eval_runs" / "repair-isolation-default-after" / "memoryos.db"
    ).read_text(encoding="utf-8", errors="ignore")
...
def test_repair_smoke_archive_artifacts_are_visible_only_when_session_attached(tmp_path):
    ...
    default_eligibility = default_report["v3_context"]["metadata"]["archival_eligibility"]
    assert default_report["repair_smoke"]["enabled"] is False
...
def test_repair_smoke_comparison_report_lists_case_level_movement_and_source_metrics(tmp_path):
            ...
            [
                {
                    ...
                    "model_visible_planner_input": {
                        ...
                        "selected_context_ids": ["case_fail_to_pass_qa_001:case_fail_to_pass:D1:1"],
                        "final_context_trace_source_ids": [
                        ...
                        "cited_source_ids": ["case_fail_to_pass_qa_001:case_fail_to_pass:D1:1"],
                        "unsupported_citation_ids": [],
                    },
                    "eval_gold_sidecar": {
                        "case_id": "case_fail_to_pass_qa_001",
                        "expected_answer": "emerald",
                        "expected_source_ids": ["case_fail_to_pass_qa_001:case_fail_to_pass:D1:1"],
                        "verdict": "fail",
                        "judge_status": "judge_fail",
                        "failure_class": "retrieval_miss",
                        "movement_status": "unchanged_fail",
                ...
                },
                {
                    ...
                    "model_visible_planner_input": {
                        ...
                        "selected_context_ids": ["case_pass_to_fail_qa_001:case_pass_to_fail:D1:1"],
                        "final_context_trace_source_ids": [
                        ...
                        "cited_source_ids": ["case_pass_to_fail_qa_001:case_pass_to_fail:D1:1"],
                        "unsupported_citation_ids": [],
                    },
                    "eval_gold_sidecar": {
                        "case_id": "case_pass_to_fail_qa_001",
                        "expected_answer": "violet",
                        "expected_source_ids": ["case_pass_to_fail_qa_001:case_pass_to_fail:D9:9"],
                        "verdict": "pass",
                        "judge_status": "judge_pass",
    ...
    summary_path = (
        settings.data_dir / "evals" / "repair-comparison-summary_locomo_repair_smoke_summary.json"
    )
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    ...
    assert summary["failure_classes"]["retrieval_miss"] == ["case_pass_to_fail_qa_001"]
    assert summary["source_metric_movement"]["source_hit"]["improved"] == [
    ...
    for metric in (
        ...
        assert summary["source_metric_movement"][metric]["improved"] == ["case_fail_to_pass_qa_001"]
        assert summary["source_metric_movement"][metric]["regressed"] == [
            "case_pass_to_fail_qa_001"
        ]
...
def test_repair_smoke_summary_buckets_judge_pass_source_miss_separately():
    ...
    assert summary["failure_classes"]["source_miss_judge_pass"] == ["judge_pass_source_miss"]
...
def test_repair_smoke_summary_blocks_missing_extra_and_duplicate_baseline_rows():
    ...
    assert "baseline report must contain exactly one row" in summary["full_chain_gate_reason"]
...
def test_no_llm_repair_smoke_report_is_diagnostic_not_full_chain_gate():
    ...
    assert provider_blocked_summary["full_chain_gate_status"] == "blocked_provider_unavailable"
...
def test_full_chain_repair_smoke_provider_blocker_writes_summary(tmp_path):
    ...
    summary = json.loads(
        (
            settings.data_dir / "evals" / "repair-provider-blocked_locomo_repair_smoke_summary.json"
        ).read_text(encoding="utf-8")
    )
...
def test_public_report_emits_planner_artifacts_without_planner_tool_execution(tmp_path):
    ...
    report = results[0].to_report()
    assert report["model_visible_planner_input"]["question"] == ("What is the planner marker?")
    ...
    assert all(event["event_type"] != "tool_executed" for event in report["kernel_trace_events"])
