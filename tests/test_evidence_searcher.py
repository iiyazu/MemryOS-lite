...
def _c(msg_id, index_text, original=None):
    return EvidenceCandidate(
        ...
        original_text=original or index_text,
        display_text=original or index_text,
    )
...
def test_contextual_prefix_helps():
    s = EvidenceSearcher()
    ctx_prefix = (
        "[session=s1] Previous: What is Caroline's relationship status? Current: She is single now."
    )
    hits = s.search(
        [
        ...
        query="Caroline relationship status",
        top_k=2,
    )
