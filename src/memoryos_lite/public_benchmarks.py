...
def run_public_benchmark(
    ...
    if report_path.exists():
        rows = _load_partial_report_rows(report_path)
        return [_public_result_from_report_row(row) for row in rows]

    resumed_rows: list[dict[str, Any]] = []
    if partial_report_path.exists():
        resumed_rows = _load_partial_report_rows(partial_report_path)
        completed_cases = _completed_case_count(
            resumed_rows,
            baseline_count=len(expanded_baselines),
        )
        if completed_cases:
            public_cases = public_cases[completed_cases:]

    results: list[PublicBenchmarkResult] = [
        _public_result_from_report_row(row) for row in resumed_rows
    ]
...
def _load_partial_report_rows(report_path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise ValueError("public benchmark report must be a JSON list")
    return [row for row in rows if isinstance(row, dict)]


def _completed_case_count(
    ...
    *,
    ...
    if baseline_count <= 0:
        return 0
    if len(rows) % baseline_count != 0:
        raise ValueError("partial report row count is not aligned to completed cases")
    return len(rows) // baseline_count


def _public_result_from_report_row(row: dict[str, Any]) -> PublicBenchmarkResult:
    payload = dict(row)
...
def _normalize_answer_citations(answer: str, evidence: list[AnswerEvidence]) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        matches = [allowed_id for allowed_id in allowed_ids if allowed_id.endswith(f":{citation}")]
        if len(matches) == 1:
            return f"[{matches[0]}]"
        return match.group(0)
...
def _repair_missing_preference_support(
    ...
    normalized_answer = " ".join(answer.lower().split())
    if not any(
        marker in normalized_question for marker in ("would", "career option", "pursue", "prefer")
    ):
        return answer
