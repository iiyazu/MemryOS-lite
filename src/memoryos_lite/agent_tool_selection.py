...
class ToolSelectionBoundary:
    ...
    def resolve(
        ...
        if not candidates:
            reason = (
                str(rejected_inputs[0]["reason"]) if rejected_inputs else "no executable candidates"
            )
    ...
    def _candidate_for_request(
        ...
        elif request.tool_name == "core_candidate_apply":
            candidate_id = str(arguments.get("candidate_id") or "").strip()
            if not candidate_id:
                raise ValueError("core_candidate_apply candidate requires candidate_id")
            actor = str(arguments.get("actor") or "agent").strip() or "agent"
            arguments.update({"candidate_id": candidate_id, "actor": actor})
            constraints = {
                "allowed_tool": "core_candidate_apply",
                "requires_source_refs_or_approval": spec.requires_source_refs_or_approval,
                "applies_core_memory": True,
