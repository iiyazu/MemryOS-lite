...
from memoryos_lite.recovery import RecoveryEvent
...
class IncompleteLineageMergeError(RuntimeError):
    ...
    def __init__(self, pending_lineages: list[str]) -> None:
        ...
        super().__init__(
            "graph lineage merge coordination pending: " + ", ".join(self.pending_lineages)
        )
...
def memory_observe_node_fn(state: AgentState) -> AgentState:
    ...
    if not obs["success"]:
        return _state_with(state, observation_summary=f"error: {obs['error']}")
    parts: list[str] = []
        ...
        service.trace(
            ...
            "agent_lineage_merged",
            {
                **current_observability_context(),
                ...
                "merged_lineage_ids": [
                    record["lineage_id"] for record in result.get("merged_lineages", [])
                ],
