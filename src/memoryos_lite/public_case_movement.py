    ...
    for path in paths:
        ...
        for row in rows:
            ...
            comparison[(benchmark, baseline, case_id)] = BaselineCaseVerdict(
                ...
                episode_source_hit_at_10=_bool_or_none(row.get("episode_source_hit_at_10")),
            )
    return comparison
...
def build_public_case_movement_summary(
    ...
    failure_classes: dict[str, list[str]] = {key: [] for key in FAILURE_CLASS_KEYS}
    failure_boundaries: dict[str, list[str]] = {key: [] for key in FAILURE_BOUNDARY_KEYS}
    source_metric_movement: dict[str, dict[str, list[str]]] = {
        metric: {key: [] for key in SOURCE_METRIC_MOVEMENT_KEYS} for metric in SOURCE_METRIC_KEYS
    }
    total_cases = 0
    ...
    return {
        ...
        "counts": {
            ...
            "failure_classes": {key: len(value) for key, value in failure_classes.items()},
            "failure_boundaries": {key: len(value) for key, value in failure_boundaries.items()},
            "source_metric_movement": {
                metric: {key: len(value) for key, value in metric_movement.items()}
...
def _row_baseline_source_metrics(row: dict[str, Any]) -> dict[str, bool | None]:
    ...
    if not isinstance(metrics, dict):
        return {}
    return {metric: _bool_or_none(metrics.get(metric)) for metric in SOURCE_METRIC_KEYS}
