"""Framework-neutral recovery primitives for MemoryOS and xmuse integrations."""
...
class CircuitOpenError(RuntimeError):
    ...
    def __init__(self, component: str, retry_after_s: float) -> None:
        ...
        super().__init__(f"circuit open for {component}; retry after {self.retry_after_s:.3f}s")
...
class RecoveryConfig:
    ...
    @classmethod
    def from_env(cls, prefix: str = "MEMORYOS_RECOVERY") -> RecoveryConfig:
        return cls(
    ...
    def delay_for_attempt(self, failed_attempt: int) -> float:
        ...
        if failed_attempt <= 0:
            return 0.0
        delay = self.initial_delay_s * (self.backoff_multiplier ** max(0, failed_attempt - 1))
