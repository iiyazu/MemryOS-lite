...
class RecallPipeline:
    ...
    def build_context(
        ...
        indexed_source_ids = sorted(
            {source_id for episode in recall_entries for source_id in episode.source_message_ids}
        )
    ...
    def _candidate_hit_to_cache(hit: EpisodeHit) -> dict[str, object]:
        return {
            ...
            "diagnostics": [diagnostic.model_dump(mode="json") for diagnostic in hit.diagnostics],
        }
            ...
            for raw_hit in raw_hits:
                ...
                hits.append(
                    EpisodeHit(
                        ...
                        diagnostics=tuple(
                            DiagnosticEvent.model_validate(diagnostic) for diagnostic in diagnostics
                        ),
                        rank_features={
                            str(key): float(value) for key, value in rank_features.items()
                        },
                        neighbor_of=(
    ...
    def _session_ids_from_hits(hits: list[EpisodeHit]) -> list[str]:
        ...
        for hit in hits:
            session_id = RecallPipeline._hit_temporal_scope(hit).get("benchmark_session_id")
            ...
            packets.append(
                {
                    ...
                    "packet_anchor_message_id": hit.packet_metadata.get("packet_anchor_message_id"),
