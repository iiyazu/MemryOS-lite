...
class RecallMemorySearcher:
    ...
    def search(
        ...
        if preserve_neighbors:
            for hit in direct_selected:
                benchmark_session_id = hit.episode.temporal_scope.get("benchmark_session_id")
            ...
            if not expandable_session_ids and len(direct_selected) < top_k:
                ...
                expandable_session_ids = {
                    session_id for session_id, count in direct_session_counts.items() if count >= 2
                }
        neighbor_limit = (
            top_k if not preserve_neighbors else top_k + top_k * (before_window + after_window)
        )
        for hit in direct_selected:
    ...
    def _select_direct_hits(
        ...
        *,
        ...
        selected_rank = {hit.episode.message_id: index for index, hit in enumerate(direct_hits)}
        selected.sort(key=lambda hit: selected_rank.get(hit.episode.message_id, len(direct_hits)))
        ...
        remaining = [hit for hit in direct_hits if hit.episode.message_id not in selected_ids]
    ...
    def _add_neighbors(
        ...
        for offset in range(1, max(neighbors_before, neighbors_after) + 1):
            ...
            for position in positions:
                ...
                if neighbor is None:
                    continue
                hit_benchmark_session = hit.episode.temporal_scope.get("benchmark_session_id")
                neighbor_benchmark_session = neighbor.temporal_scope.get("benchmark_session_id")
                if (
