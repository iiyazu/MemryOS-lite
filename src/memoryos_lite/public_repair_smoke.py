...
def build_executable_repair_proposal(
    ...
    if (
        ...
        or tool_name not in executable_kernel_tool_names()
    ):
...
def _forbidden_values(row: dict[str, Any]) -> set[str]:
    ...
    if isinstance(sidecar, dict):
        ...
        if isinstance(expected_source_ids, list):
            values.update(
                value for value in expected_source_ids if isinstance(value, str)
            )
...
def _rewrite_aliases(value: Any, source_id_aliases: dict[str, str]) -> Any:
    if isinstance(value, dict):
        return {
            key: _rewrite_aliases(item, source_id_aliases)
        ...
        artifacts.append(
            {
                ...
                "session_attachment_found": bool(
                    verification.get("session_attachment_found")
                ),
...
def build_repair_smoke_comparison_summary(
    ...
    *,
    ...
    summary["counts"] = {
        ...
        "failure_classes": {
            key: len(value)
            for key, value in summary["failure_classes"].items()
        },
        "source_metric_movement": {
            metric: {
                direction: len(cases)
                for direction, cases in movement.items()
            }
        ...
        "baseline_coverage": {
            ...
            "missing_baseline_case_ids": len(
                baseline_coverage["missing_baseline_case_ids"]
            ),
            "extra_baseline_case_ids": len(
                baseline_coverage["extra_baseline_case_ids"]
            ),
            "duplicate_baseline_case_ids": len(
                baseline_coverage["duplicate_baseline_case_ids"]
            ),
            "duplicate_repair_case_ids": len(
                baseline_coverage["duplicate_repair_case_ids"]
            ),
        },
    }
    return summary
...
def _baseline_coverage(
    ...
    matched_case_ids = sorted(
        ...
        if case_id not in duplicate_baseline_case_ids
        and case_id not in duplicate_repair_case_ids
    )
