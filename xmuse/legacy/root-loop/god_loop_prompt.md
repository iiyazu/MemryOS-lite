...
启动后先写入或确认 `xmuse/work/current_goal.md`：
...
1. `xmuse/state.json`
2. `xmuse/blueprint.md`
3. `xmuse/work/**` reports relevant to the current phase
...
`xmuse/work/` if needed, then exit cleanly unless the state or user
...
Reuse the current `xmuse` infrastructure. This is a prompt-and-blueprint
...
Parallel feature state lives in optional `xmuse/feature_lanes.json`, not
...
1. Read `xmuse/feature_lanes.json` when present.
...
- active goal from `xmuse/work/current_goal.md`;
...
Before dispatch, check that `xmuse/config.json` phase metadata points to
