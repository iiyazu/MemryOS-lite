...
`master_state.features[]` entry, `xmuse/prompts/slave_god_prompt.md`,
`xmuse/contracts/slave_dispatch_template.json`,
`xmuse/work/features/<feature-id>/slave_state.json`, and
`xmuse/work/features/<feature-id>/blueprint.md` before acting.
...
- continue autonomous repair after feature-local FAIL review or non-usable ACK;
- produce `result.md`, `ack.json`, and `review_verdict.json`;
- propose feature amendments for Master review when scope must change;
...
All feature-local Slave, plan, execute, and review nodes must run without
interactive shell/tool confirmation. Launch them through
...
do not wait for human confirmation during feature-local planning, execution,
review, ACK production, rework, or repair. Convert confirmation prompts into
feature-local written decisions or amendment proposals.

This policy does not authorize self-signed external merge approval, edits
outside the assigned worktree, destructive cleanup, deletion of unrelated user
work, or lowering gates.
...
When `review_verdict.json` is FAIL, `ack_level` is not usable, or Master writes
a rework request, Slave should continue autonomous repair inside the feature
boundary. Use `repairing`, `reworking`, `feature_blocked`, or `active_repair`
for feature-local states that still belong in active work.

Each repair iteration must start from concrete engineering blockers: failing
tests, type errors, missing real-path implementation, broken source grounding,
default-regression risk, incomplete artifacts, or a review finding. Benchmark
scores are not benchmark score targets and must not become repair goals. Use
benchmark or eval output only to diagnose case-level failures, regressions, or
missing evidence.

Stop local repair and propose a Master decision only when the blocker is outside
Slave authority: cross-feature conflict, target-branch integration, external
approval, unavailable dependency, required scope split, or a policy question
that would lower gates.

## plan/execute/review Nodes

Plan node:

- writes only planning artifacts under `xmuse/work/features/<feature-id>/`;
...
- must write feature-local `execute_goal.md` with an explicit `/goal`, real
  paths, required artifacts, non-goals, demo-only prohibitions, diagnostic-only
  benchmark wording, and `Max repair cycles: 1-3` before changing code;
...
Slave may propose feature amendments in feature-local artifacts when the current
blueprint is too broad, too narrow, blocked, or should be split. A proposal is
not active control-plane state: Slave must not edit `master_state.json`, must
not write `xmuse/master/amendments/`, and must not lower any gate.
