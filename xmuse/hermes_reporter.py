...
LOOP = Path("/home/iiyatu/projects/python/memoryOS/xmuse")
PROJECT = LOOP.parent
...
def master_slave_report():
    ...
    try:
        ...
        if summary.get("source") != "xmuse/master_state.json":
...
def active_job_liveness(*, read_only: bool = False) -> str:
...
def is_god_alive(grace_period: bool = False, *, read_only: bool = False) -> bool:
    """God is alive if lock PID exists AND (heartbeat fresh OR in grace period)."""
    if lock_status() != "ok":
        return False
    active_job = active_job_liveness(read_only=read_only)
...
def generate_report(s, *, read_only: bool = False):
    now = datetime.now(UTC)
    god_alive = is_god_alive(read_only=read_only)
    ...
    report = {
        ...
        "stale": not is_god_alive(read_only=read_only) and lock_status() in ("stale", "ok"),
        "god": {
            "alive": god_alive,
...
def main():
    ...
    report_only = os.environ.get("XMUSE_REPORT_ONLY") == "1"
    god_alive = is_god_alive(read_only=report_only)
    action = "report_only" if report_only else ("done" if done else "wait")
    if not report_only and not done and not god_alive:
        started = start_god()
        god_alive = started
        action = "started_ok" if started else "started_failed"

    # Generate report (report uses is_god_alive() without grace)
    report = generate_report(s, read_only=report_only)
